...
try:
    while True:
        ...
        if ret_encode:
            try:
                sock.sendto(jpeg.tobytes(), ('10.42.0.255', UDP_PORT))
            except Exception:
                pass 
        ...
        print(f"FPS: {fps:4.1f} | Cap:{cap_ms:4.1f}ms | Prep:{pre_ms:4.1f}ms | AI:{ai_ms:5.1f}ms | Trk:{trk_ms:4.1f}ms | Draw:{drw_ms:4.1f}ms | UDP:{udp_ms:4.1f}ms")
