...
def send_angle_to_crazyflie(ser, angle_deg):
    # 'S' header + float
    packet = struct.pack('<cf', b'S', angle_deg)
    ser.write(packet)
    ser.flush() 
...
try:
    while True:
        ...
        if len(tracked_objects) > 0:
            ...
            for obj in tracked_objects:
                ...
                if is_target:
                    ...
                    if ENABLE_CRAZYFLIE:
                        send_angle_to_crazyflie(ser, angle_deg)
        ...
        print(f"[{status}] FPS: {fps:.1f} | Detections: {len(tracked_objects)} | Angle: {current_angle:>5.1f}°      ", end='\r')
