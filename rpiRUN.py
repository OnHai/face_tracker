...
import socket
from picamera2 import Picamera2  # <--- 1. NEW IMPORT
...
try:
    while True:
        # --- 3. NEW CAPTURE METHOD ---
        frame = picam2.capture_array()
        if frame is None:
            continue
            
        # Convert the mono array to BGR so we can draw colored boxes on it
            ...
            for obj in tracked_objects:
                ...
                if obj_id == locked_target_id:
                    ...
                    if abs(angle_degrees) > deadzone:
                        ...
                        print(f"Command: angle {angle_degrees:.2f} |  ID: {locked_target_id}")
                    else:
        ...
        if ret_encode:
            try:
                # EVERYONE 
                sock.sendto(jpeg.tobytes(), ('255.255.255.255', UDP_PORT))
            except Exception:
                pass 
